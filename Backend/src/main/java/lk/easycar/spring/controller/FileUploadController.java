    ...
    public ResponseUtil uploadCarViews(@RequestPart("front") MultipartFile front,@RequestPart("rear") MultipartFile rear, @RequestPart("side") MultipartFile side,@RequestPart("interior") MultipartFile interior,@RequestParam("fleet") String carFleet,@RequestParam("brand") String brand) {
        try {
            ...
            if (carFleet.equals("General")) {
                ...
                File brandDir = new File(uploadsDir + "/General Cars/"+brand);
                System.out.println("brandDir : " + brandDir); // --> /opt/apache-tomcat-8.5.78/webapps/easycar/uploads/cars/General Cars/Suzuki
                brandDir.mkdir();

                front.transferTo(new File(generalDir.getAbsolutePath() + "/" + front.getOriginalFilename()));
                rear.transferTo(new File(generalDir.getAbsolutePath() + "/" + rear.getOriginalFilename()));
                side.transferTo(new File(generalDir.getAbsolutePath() + "/" + side.getOriginalFilename()));
                interior.transferTo(new File(generalDir.getAbsolutePath() + "/" + interior.getOriginalFilename()));


                allImages.add("uploads/cars/General Cars/"+brand + front.getOriginalFilename());
                allImages.add("uploads/cars/General Cars/"+brand + rear.getOriginalFilename());
                allImages.add("uploads/cars/General Cars/"+brand + side.getOriginalFilename());
                allImages.add("uploads/cars/General Cars/"+brand + interior.getOriginalFilename());
